...
public sealed class UserService : IUserService
{
    ...
    public async Task<ResponseModel> GetUsersAsync(UserQueryModel request)
    {
        ...
        if (!string.IsNullOrEmpty(request.Role)) query = query.Where(u => string.Equals(u.Role!.Name, request.Role));
    ...
    public async Task<ResponseModel> ChangePasswordAsync(Guid userId, ChangePasswordModel model)
    {
        ...
        if (user == null) return new ResponseModel { Message = "User not found", Status = "Error" };
