...
public class PaymentService : IPaymentService
{
    ...
    public async Task<ResponseModel> CreatePaymentLink(int orderCode)
    {
        try
        {
            ...
            {
                return ResponseModel.Error("Không tìm thấy thông tin khách hàng. Vui lòng kiểm tra lại tài khoản");
            }
            var items = order.OrderDetails.Select(
                item =>
                    new ItemData(
                        item.Product.Name,
                        item.Quantity,
            ...
            var description = $"Phí vận chuyển:{(int)order.ShippingFee}đ";
    ...
    public async Task<ResponseModel> GetPaymentLinkInformation(int orderCode)
    {
        try
        {
            ...
            {
                return ResponseModel.Error("Đã có lỗi xảy ra trong quá trình lấy thông tin link thanh toán");
            }
            return ResponseModel.Success("Lấy thông tin link thanh toán thành công", paymentLinkInformation);
        }
        catch(Exception ex)
        {
            return ResponseModel.Error(ex.Message);
