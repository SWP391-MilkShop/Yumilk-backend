...
public class OrderService : IOrderService
{
    ...
    public async Task<ResponseModel> GetOrderAsync(OrderQueryModel model)
    {
        ...
        if (!string.IsNullOrEmpty(model.Email)) query = query.Where(o => string.Equals(o.Customer!.Email, model.Email));

        if (model.TotalAmount > 0) query = query.Where(o => o.TotalAmount > model.TotalAmount);

        if (model.ToOrderDate is not null) query = query.Where(o => o.CreatedAt <= model.ToOrderDate);
