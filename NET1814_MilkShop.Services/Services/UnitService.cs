...
public class UnitService : IUnitService
{
    ...
    public async Task<ResponseModel> UpdateUnitAsync(int id, UpdateUnitModel unitModel)
    {
        ...
        if (isExistUnit == null) return new ResponseModel { Status = "failed", Message = "Unit not found" };

        if (!unitModel.Name.IsNullOrEmpty()) isExistUnit.Name = unitModel.Name;

        if (!unitModel.Description.IsNullOrEmpty()) isExistUnit.Description = unitModel.Description;
        if (unitModel.IsActive.HasValue) isExistUnit.IsActive = unitModel.IsActive!.Value;
